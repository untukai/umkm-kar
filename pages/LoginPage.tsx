    ...
    <div className="flex justify-center items-center py-12 px-4">
      <div className="w-full max-w-md bg-white p-8 rounded-xl shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-8">
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-2">Masuk sebagai:</label>
            <div className="grid grid-cols-2 gap-2 rounded-lg bg-neutral-100 p-1">
              <button
                type="button"
                onClick={() => setRole('pembeli')}
                className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${role === 'pembeli' ? 'bg-primary text-white shadow' : 'text-neutral-600 hover:bg-neutral-200'}`}
              >
                Pembeli
              </button>
              <button
                ...
                className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${role === 'penjual' ? 'bg-primary text-white shadow' : 'text-neutral-600 hover:bg-neutral-200'}`}
              >
                Penjual
              </button>
            </div>
