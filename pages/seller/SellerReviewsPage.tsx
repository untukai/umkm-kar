    ...
    <div className="border rounded-lg p-4">
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-3">
          <img src={product?.imageUrls[0]} alt={product?.name} className="w-12 h-12 rounded-md object-cover bg-neutral-200" />
          <div>
            <p className="font-semibold text-neutral-800">{product?.name || 'Produk Dihapus'}</p>
      ...
      <div className="pl-15 mt-2">
        ...
        <p className="text-neutral-700 text-sm italic bg-neutral-50 p-3 rounded-md">"{review.comment}"</p>
...
const SellerReviewsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold">Rating & Ulasan</h1>
        <p className="text-neutral-600 mt-1">Lihat dan balas ulasan dari pelanggan Anda.</p>
      </div>
      
      {/* Summary */}
      <div className="bg-white p-6 rounded-lg shadow-md">
        <h3 className="text-lg font-bold">Ringkasan Ulasan</h3>
        <div className="flex items-center gap-4 mt-3">
          <p className="text-4xl font-bold text-primary">{averageRating.toFixed(2)}</p>
          <div>
            <StarRating rating={averageRating} />
            <p className="text-sm text-neutral-500">dari {sellerReviews.length} ulasan</p>
          </div>
        </div>
      </div>
      ...
      {/* Review List */}
      <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-lg font-bold mb-4">Semua Ulasan</h3>
          <div className="space-y-4">
            {sellerReviews.length > 0 ? (
                sellerReviews.map(review => <ReviewCard key={review.id} review={review} />)
            ) : (
                <p className="text-center text-neutral-500 py-8">Belum ada ulasan untuk produk Anda.</p>
            )}
          </div>
      </div>
    </div>
