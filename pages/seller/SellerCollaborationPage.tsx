...
const InfluencerCard: React.FC<{ influencer: Influencer }> = ({ influencer }) => {
  ...
  return (
    <div className="bg-white border rounded-lg p-4 flex flex-col items-center text-center transition-shadow hover:shadow-xl h-full">
      <div className="w-24 h-24 rounded-full bg-neutral-200 mb-3 flex items-center justify-center flex-shrink-0">
        <UserIcon className="w-12 h-12 text-neutral-400" />
      </div>
      <h3 className="font-bold text-lg text-neutral-800">{influencer.name}</h3>
      <span className="text-xs font-semibold text-primary bg-primary/10 px-2 py-0.5 rounded-full my-1">{influencer.category}</span>
      <p className="text-sm text-neutral-600 my-2 flex-grow">{influencer.bio}</p>
      <div className="flex justify-center gap-4 text-sm w-full my-3 border-t pt-3">
        <div>
          <p className="font-bold">{formatFollowers(influencer.followers.instagram)}</p>
          <p className="text-xs text-neutral-500">Instagram</p>
        </div>
        <div>
          <p className="font-bold">{formatFollowers(influencer.followers.tiktok)}</p>
...
const SellerCollaborationPage: React.FC = () => {
    ...
    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-2xl font-bold">Kolaborasi dengan Influencer</h1>
                <p className="text-neutral-600 mt-1">Temukan influencer yang tepat untuk mempromosikan produk Anda.</p>
            </div>
            ...
            <div className="bg-white p-4 rounded-lg shadow-md">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    />
                    <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-white">
                        <option value="all">Semua Kategori</option>
                        {categories.map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}
                    </select>
                    <select value={followerRange} onChange={e => setFollowerRange(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-white">
