    ...
    <div className="space-y-16">
      <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg">
       ...
       <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg">
            ...
            <h2 className="text-2xl font-bold text-center mb-8">Tim Pendiri</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {founders.map(founder => (
                <div key={founder.name} className="flex flex-col items-center text-center">
                  <div className="w-32 h-32 rounded-full bg-neutral-200 mb-4 flex items-center justify-center">
                    <UserIcon className="w-16 h-16 text-neutral-400" />
                  </div>
                  <h3 className="text-lg font-bold text-neutral-800">{founder.name}</h3>
                  <div className="flex items-center gap-4 mt-3">
                    <a href={founder.contact.email} className="text-neutral-500 hover:text-primary transition-colors" aria-label={`Email ${founder.name}`}>
                      <MailIcon className="w-6 h-6" />
                    </a>
                    <a href={founder.contact.linkedin} className="text-neutral-500 hover:text-primary transition-colors" aria-label={`LinkedIn ${founder.name}`}>
                      <LinkedInIcon className="w-6 h-6" />
                    </a>
                    <a href={founder.contact.twitter} className="text-neutral-500 hover:text-primary transition-colors" aria-label={`Twitter ${founder.name}`}>
